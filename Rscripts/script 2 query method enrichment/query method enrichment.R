...
export(tmp4,"24470 gene annotation.xlsx")
...
#Fibroblast foci signature.txt  n = 50 to 50
...
export(tmp7, "Fibroblast foci signature.txt")


#Distal alveoli signature.txt n = 41 to 41
...
export(tmp7, "Distal alveoli signature.txt")


#Immune infiltrate signature.txt n = 111 to 106
...
export(tmp7, "Immune infiltrate signature.txt")


#IPF blood vessel signature.txt n = 28 to 27
...
export(tmp7, "IPF blood vessel signature.txt")


#healthy alveoli signature.txt n = 32
...
export(tmp7, "Healthy alveoli signature.txt")
...
ff_signature <- read_table("Fibroblast foci signature.txt",     
                           col_names = TRUE)
...
# distal alveoli
distal_alv_signature <- read_table("Distal alveoli signature.txt", 
                           col_names = TRUE)

distal_alv_sig_count <- normalized_count[c(distal_alv_signature$index),]  ## 41 gene * 89326 cells

# immune infiltrate
imm_infil <- read_table("Immune infiltrate signature.txt", 
                                   col_names = TRUE)

imm_infil_sig_count <- normalized_count[c(imm_infil$index),]  ## 106 gene * 89326 cells

# ipf blood vessel
ipf_vessel_signature <- read_table("IPF blood vessel signature.txt", 
                                   col_names = TRUE)
...
healthy_alv_signature <- read_table("Healthy alveoli signature.txt", 
                                   col_names = TRUE)
...
export(summary_signature_table,  "query enrichment score summary table.xlsx")
...
# save bubble plot
ggsave("query positive enrichment bubble plot summary.pdf", plot = p1, width = 8.5, height = 11)  
ggsave("query negative depletion bubble plot summary.pdf", plot = p2, width = 8.5, height = 11)  


# save data
save.image("query method enrichment.RData")
